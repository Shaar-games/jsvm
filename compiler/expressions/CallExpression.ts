...
async function compileCallExpression(node, context) {
  ...
  if (node.callee.type === "Super") {
    ...
    if (!hasSpread) {
      ...
      emit(context, [OpCode.SUPER_CALL, returnRegister, argumentRegisters.length, ...argumentRegisters]);
    ...
    const returnRegister = newRegister(context);
    emit(context, [OpCode.SUPER_CALLSPREAD, returnRegister, argsArrayRegister]);
    return returnRegister;
  }
  ...
  if (node.callee.type === "MemberExpression" && node.callee.object.type === "Super") {
    ...
    thisRegister = newRegister(context);
    emit(context, [OpCode.SUPER_GET, functionRegister, propertyRegister]);
    emit(context, [OpCode.LOAD_THIS, thisRegister]);
